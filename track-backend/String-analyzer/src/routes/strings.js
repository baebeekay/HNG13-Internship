...
router.get('/filter-by-natural-language', async (req, res) => {
  try {
    ...
    if (filters.max_length) where.length = { ...where.length, [Op.lte]: filters.max_length || Infinity };
...
router.get('/', async (req, res) => {
  try {
    ...
    const parseLength = (val) => (val ? parseInt(val) : undefined);
    if (minLength !== undefined) where.length = { ...where.length, [Op.gte]: parseLength(minLength) || 0 };
    if (maxLength !== undefined) where.length = { ...where.length, [Op.lte]: parseLength(maxLength) || Infinity };
